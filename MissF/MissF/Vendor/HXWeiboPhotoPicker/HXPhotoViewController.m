...
- (void)setup {
    ...
    //self.navigationController.navigationBar.tintColor = [UIColor blackColor];//wyao 自定义修改
    ...
    [self.navigationItem.leftBarButtonItem setTitleTextAttributes:@{NSFontAttributeName:FONT(15), NSForegroundColorAttributeName:WHITECOLOR} forState:UIControlStateNormal];//wyao  自定义修改
    if (!self.manager.singleSelected) {
        ...
        if (self.manager.selectedList.count > 0) {
            ...
            //[self.rightBtn setBackgroundColor:[UIColor colorWithRed:253/255.0 green:142/255.0 blue:36/255.0 alpha:1]];//wyao 自定义修改
            ...
            [self.rightBtn setBackgroundColor:[UIColor clearColor]];//wyao 自定义修改
    ...
    [titleBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];//wyao 自定义颜色
    [titleBtn setImage:[HXPhotoTools hx_imageNamed:@"headlines_icon_arrow"] forState:UIControlStateNormal];
    titleBtn.titleLabel.font = [UIFont systemFontOfSize:15.0f];
    ...
    if (!self.manager.singleSelected) {
        ...
        HXPhotoBottomView *bottomView = [[HXPhotoBottomView alloc] initWithFrame:CGRectMake(0, height-50-64, width, 50)];
        bottomView.backgroundColor = BACKGROUNDCOLOR;//自定义背景颜色
...
//            }
        ...
        self.navigationItem.rightBarButtonItem.enabled = YES;
        self.rightBtn.hidden = NO;//自定义 当未选中的时候显示按钮
        [self.rightBtn setTitle:[NSString stringWithFormat:@"下一步(%ld)",self.manager.selectedList.count] forState:UIControlStateNormal];
        //[self.rightBtn setBackgroundColor:[UIColor colorWithRed:253/255.0 green:142/255.0 blue:36/255.0 alpha:1]];//wyao  自定义修改
        [self.rightBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];//wyao 自定义按钮不能选中的颜色
        ...
        self.bottomView.originalBtn.enabled = NO;
        self.navigationItem.rightBarButtonItem.enabled = NO;
        self.rightBtn.hidden = YES;//自定义 当未选中的时候隐藏按钮
        //[self.rightBtn setTitle:@"下一步" forState:UIControlStateNormal];
        //[self.rightBtn setBackgroundColor:[UIColor whiteColor]];
        self.rightBtn.frame = CGRectMake(0, 0, 60, 25);
        //self.rightBtn.layer.borderWidth = 0.5;
...
- (UIButton *)rightBtn
{
    if (!_rightBtn) {
        ...
        */
        ...
        [_rightBtn setTitleColor:[[UIColor whiteColor] colorWithAlphaComponent:0.5] forState:UIControlStateHighlighted];
        //******************* wyao自定义修改******************
...
- (void)setup {
    ...
    [originalBtn setImage:[HXPhotoTools hx_imageNamed:@"origin_select@2x.png"] forState:UIControlStateSelected];//wyao 自定义原图选择图标
