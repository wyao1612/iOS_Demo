...
- (void)setup
{
    ...
    if (self.selectedComplete) {
        ...
        [navItem.leftBarButtonItem setTitleTextAttributes:@{NSFontAttributeName:FONT(15), NSForegroundColorAttributeName:WHITECOLOR} forState:UIControlStateNormal];
...
- (UILabel *)titleLb
{
    if (!_titleLb) {
        ...
        _titleLb.textColor = [UIColor whiteColor];//wyao 自定义照片预览titile
...
- (UIButton *)selectedBtn
{
    if (!_selectedBtn) {
        ...
        [_selectedBtn setImage:[HXPhotoTools hx_imageNamed:@"picker_select@2x.png"] forState:UIControlStateSelected];//wyao自定义修改
...
- (void)didSelectedClick:(UIButton *)button
{
    ...
    if (self.manager.selectedList.count > 0) {
        self.navigationItem.rightBarButtonItem.enabled = YES;
        self.rightBtn.hidden = NO; //wyao 自定义 显示
        [self.rightBtn setTitle:[NSString stringWithFormat:@"下一步(%ld)",self.manager.selectedList.count] forState:UIControlStateNormal];
        //[self.rightBtn setBackgroundColor:[UIColor colorWithRed:253/255.0 green:142/255.0 blue:36/255.0 alpha:1]];
        //self.rightBtn.layer.borderWidth = 0;
        CGFloat rightBtnH = self.rightBtn.frame.size.height;
        ...
        self.rightBtn.hidden = YES;//wyao 自定义 隐藏
        [self.rightBtn setTitle:@"下一步" forState:UIControlStateNormal];
        //[self.rightBtn setBackgroundColor:[UIColor colorWithRed:253/255.0 green:142/255.0 blue:36/255.0 alpha:1]];
        self.rightBtn.frame = CGRectMake(0, 0, 60, 25);
        //self.rightBtn.layer.borderWidth = 0;
...
- (UIButton *)rightBtn {
    if (!_rightBtn) {
        ...
        [_rightBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateDisabled];//wyao 自定义颜色  lightGrayColor
        [_rightBtn setTitleColor:[[UIColor whiteColor] colorWithAlphaComponent:0.5] forState:UIControlStateHighlighted]; //wyao 自定义颜色  lightGrayColor
        /*
