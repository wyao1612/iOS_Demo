...
- (void)FetchAllPhotoForPHFetchResult:(PHFetchResult *)result Index:(NSInteger)index FetchResult:(void(^)(NSArray *photos, NSArray *videos, NSArray *Objs))list
{
    ...
    if (self.openCamera) {
        ...
        if (photoAy.count == 0 && videoAy.count != 0) {
            ...
            model.thumbPhoto = [HXPhotoTools hx_imageNamed:@"icon_camera@2x.png"];
        }else {
            model.thumbPhoto = [HXPhotoTools hx_imageNamed:@"icon_camera@2x.png"];//compose_photo_photograph 自定义相机视图
        }
